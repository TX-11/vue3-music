 ...
 * @returns
 */
export async function getPhoneCaptcha(phone: string, ctcode?: string) {
	return await http.get<{
		code: number;
		cookie: string;
		token: string;
	}>('/captcha/sent', { phone: phone, ctcode: ctcode });
}
 ...
 * @returns
 */
export async function verifyCaptcha(phone: string, captcha: string, ctcode?: string) {
	return await http.get<{
		code: number;
		cookie: string;
		token: string;
...
export async function useVideoGroup(id?: number, offset?: number) {
	const { datas } = await http.get<{ datas: Video[] }>(id || id === 0 ? 'video/group' : 'video/timeline/all', {
		id: id,
		offset: offset,
	});
	return datas;
